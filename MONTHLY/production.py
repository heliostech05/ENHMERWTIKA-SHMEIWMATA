...
def calculate_daily_summary_quarterly(df_prod, df_dam_15m, producer_row, month):
    ...
    try:
        ...
        for D in days:
            ...
            # DST_SKIP_DATE: πετάμε END 00:15/00:30/00:45
            if D == dst_skip:
                mask_skip = (
            ...
            # DST_FALLBACK_DATE: intervals 03:00–04:00 → κρατάμε πρώτη εμφάνιση
            if D == dst_fallback:
                mask_win = (
            ...
            # DST_FALLBACK_DATE DAM: διπλά 03:00–03:45 → κρατάμε πρώτη εμφάνιση
            if D == dst_fallback:
                mask_dam_win = (
